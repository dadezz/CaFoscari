...
* rank: a factor with levels `AssocProf` `AsstProf` `Prof` (più o meno corrispondenti, nel sistema italiano, a Ricercatore, Professore Associato e Professore Ordinario)
...
```{r}
plot(as.numeric(salaries$sex == "Male"), salaries$salary)
...
In realtà quando abbiamo predittori categorici tupicamente si confrontano i due gruppi con dei boxplot:
...
3. Abbiamo visto che `yrsService` ha un forte effetto: vediamo se aggiungere la variabile categoriale `rank` in maniera additiva o in un'interazione con `yrsService` migliora la bontà di adattamento del modello: 
...
```{r}
# frequency table
ctab <- table(salaries$discipline, salaries$sex,dnn = c("discipline", "sex"))
# divide by row totals 
signif(ctab/rowSums(ctab),3)
ctab <- table(salaries$rank, salaries$sex)
signif(ctab/rowSums(ctab),3)
par(mfrow = c(2,1))
plot(jitter(as.numeric(salaries$sex == "Male")), salaries$yrsSincePhd, pch = 16, ylab = "yrs Phd")
plot(jitter(as.numeric(salaries$sex == "Male")), salaries$yrsService, pch = 16, ylab = "yrs Service")
```
