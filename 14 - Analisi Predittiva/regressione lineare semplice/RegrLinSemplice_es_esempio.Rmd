...
Con $\varepsilon_i$ variabili casuali indipendenti ed identicamente distribuite a media 0 con una varianza costante $\sigma^2$
...
Si può ottenere un panoramica del modello stimato: 
...
Vogliamo verificare se sia possibile falsificare l'affermazione "Due uomini la cui altezza differisca di un pollice hanno in media una differenza di peso di 5 libbre". Questo corrisponde a falsificare l'affermazione $\beta_1 = 5$, cioè a testare il sistema di verifica di ipotesi: 

\[H_0: \beta_1 = 5 \quad VS \quad  H_1: \beta_1 \neq 5 \]

Per fare ciò calcoliamo la statistica test: 
...
Possiamo anche controllare i grafici usando direttamente 
...
L'interpretazione dell'intercetta nel modello originale è poco utile: non esistono persone alte 0 pollici. Se si usa invece la variabile centrata l'intercetta diventa il valore di $\bar{y}$, dato che $\bar{x} = 0$: 
